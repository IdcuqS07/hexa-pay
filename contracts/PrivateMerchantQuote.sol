...
contract PrivateMerchantQuote {
    ...
    // 1) Merchant creates encrypted quote
    function createQuote(
        ...
        // Give merchant access after storage assignment
        FHE.allow(q.amountCt, msg.sender);

        emit QuoteCreated(id, msg.sender, payer);
    }

    // 2) Merchant grants payer access for preview
    ...
    // 3) Payer settles
    // skipPreview = true => trusted merchant / blind-pay mode
    function settleQuote(bytes32 id, bool skipPreview) public {
        ...
        // FHE approval check is delegated to adapter
        credit.canSpend(msg.sender, q.amountCt);

        // Adapter is responsible for failing if insufficient credit
    ...
    // Optional manual cancel before payment
    ...
    // Optional handle getter for frontend decrypt flow
    function getEncryptedAmount(bytes32 id) external view returns (euint64) {
        Quote storage q = quotes[id];
        if (q.merchant == address(0)) revert QuoteNotFound();
        return q.amountCt;
    }
